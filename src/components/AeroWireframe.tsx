...
function SR71Model() {
    ...
    const [wireframeData, setWireframeData] = useState<{ vertices: number[]; indices: number[] } | null>(null);

    // Load pre-baked wireframe JSON
    useEffect(() => {
        fetch("/sr71-wireframe.json")
            .then((res) => res.json())
            .then((data) => setWireframeData(data))
            .catch((err) => console.error("Failed to load wireframe:", err));
    }, []);

    // Build line geometry from pre-baked data
    const lineSegments = useMemo(() => {
        if (!wireframeData) return null;

        const geometry = new THREE.BufferGeometry();
        const vertices = new Float32Array(wireframeData.vertices);
        const indices = new Uint16Array(wireframeData.indices);
